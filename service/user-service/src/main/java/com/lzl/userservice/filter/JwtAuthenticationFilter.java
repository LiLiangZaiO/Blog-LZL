...
//            ResponseUtils.write(response, HttpServletResponse.SC_FORBIDDEN, "认证服务不正常！");
            ResponseUtils.out(response, R.error().code(HttpServletResponse.SC_FORBIDDEN).message("认证服务不正常！"));
        } else if (failed instanceof UsernameNotFoundException) {
//            ResponseUtils.write(response, HttpServletResponse.SC_FORBIDDEN, "用户账户不存在！");
            ResponseUtils.out(response, R.error().code(HttpServletResponse.SC_FORBIDDEN).message("用户账户不存在！"));
        } else if (failed instanceof BadCredentialsException) {
//            ResponseUtils.write(response, HttpServletResponse.SC_FORBIDDEN, "用户密码是错的！");
            ResponseUtils.out(response, R.error().code(HttpServletResponse.SC_FORBIDDEN).message("用户密码是错的！"));
        } else if (failed instanceof AccountExpiredException) {
//            ResponseUtils.write(response, HttpServletResponse.SC_FORBIDDEN, "用户账户已过期！");
            ResponseUtils.out(response, R.error().code(HttpServletResponse.SC_FORBIDDEN).message("用户账户已过期！"));
        } else if (failed instanceof LockedException) {
//            ResponseUtils.write(response, HttpServletResponse.SC_FORBIDDEN, "用户账户已被锁！");
            ResponseUtils.out(response, R.error().code(HttpServletResponse.SC_FORBIDDEN).message("用户账户已被锁！"));
        } else if (failed instanceof CredentialsExpiredException) {
//            ResponseUtils.write(response, HttpServletResponse.SC_FORBIDDEN, "用户密码已失效！");
            ResponseUtils.out(response, R.error().code(HttpServletResponse.SC_FORBIDDEN).message("用户密码已失效！"));
